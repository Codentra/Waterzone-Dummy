    ...
    return (
      <View style={styles.card}>
        <Text style={styles.cardTitle}>
          {item.litres.toLocaleString()} L · {STATUS_LABELS[item.status] ?? item.status}
          {item.total != null ? ` · $${item.total.toFixed(2)} cash` : ""}
        </Text>
...
const styles = StyleSheet.create({
  ...
  earnings: { fontSize: 13, color: colors.primary, marginBottom: 4 },
