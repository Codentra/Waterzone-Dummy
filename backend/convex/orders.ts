...
export const createOrder = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.insert("orders", {
      ...
      notes: args.notes,
      status: "requested",
      paymentMethod: args.paymentMethod === "cash" ? "cash" : "cash",
...
export const markDelivered = mutation({
  args: {
    userId: v.id("users"),
    orderId: v.id("orders"),
    cashReceived: v.boolean(),
    cashReceivedAmount: v.optional(v.number()),
  },
  handler: async (ctx, args) => {
    await requireUserRole(ctx.db, args.userId, ["driver"]);
    if (!args.cashReceived) {
      throw new Error("Confirm cash received before marking delivered");
    }
    const driver = await ctx.db
      .query("drivers")
      .withIndex("by_user", (q) => q.eq("userId", args.userId))
    ...
    const receivedAmount = args.cashReceivedAmount ?? expectedTotal;
    if (expectedTotal > 0 && receivedAmount < expectedTotal) {
      throw new Error("Cash received is less than order total");
    }

    const config = await getActivePricingConfig(ctx.db);
    const now = Date.now();
    const dueAt = commissionDueAt(now, config.settlementCycleDays);
    await ctx.db.patch(args.orderId, {
      status: "delivered",
      deliveredAt: now,
      updatedAt: now,
